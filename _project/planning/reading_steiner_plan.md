...
- `/reading-steiner` 実行後:  
  ...
  - 世界線図ビューは Rich の Tree / Table を活用し、選択中ノードの簡易サマリを表示。  
...
## 10. リスク・課題・未確定事項
1. **記憶永続化の範囲**: 世界線メモリをファイルベースで永続化するか、セッション限定か未確定。  
2. **チェックポイント粒度**: メタコマンド実行単位 / ツール呼び出し単位 / ステップ単位いずれを採用するか要判断。  
3. **PageIndex 適用範囲**: 世界線共通 vs 世界線毎局所索引の切り替え方針未定。  
4. **パフォーマンス**: Mind Evolution + PageIndex の組込みで処理時間が増大するリスク。  
5. **UI 複雑化**: 世界線表示が煩雑になり UX が悪化する可能性。  
6. **tree コマンド未導入**: ワークスペース調査時に `tree` 不在を確認。必要なら環境へ導入検討。  
7. **外部ライセンス**: MIT だがライセンス告知や attribution の反映方法を決定する必要あり。
...
4. バタフライ効果再評価の結果は自動マージ or ユーザー承認が必要か。  
5. TUI 世界線ビューの要約表示フォーマット（例: 1行要約 + 指標スコア？）。  
6. Mind Evolution / PageIndex 取り込み時のライセンス表記ポリシー（README 追記など）の希望。
